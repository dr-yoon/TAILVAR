...
stoplost_clinvar <- stoplost_clinvar %>% mutate(TailAA_seq = paste0(Amino_acids, TailAA_seq)) %>% filter(is.na(TailAA_counts) == FALSE)
...
train_data <- rbind(stoplost_HGMD, ALFA_MAF_0.001) %>% unique()

# Testing set: ClinVar + gnomAD data
test_data <- rbind(stoplost_clinvar, gnomAD_MAF_0.001) %>% unique()
...
hydrophobic_aa <- c("A", "C", "I", "L", "M", "F", "W", "V")
...
write.table(stoplost_all, "Stoplost_SNV_input.txt", row.names = FALSE, sep = "\t", quote = FALSE)
